...
from admpo_repro.plotting import plot_figure2
...
def run_experiment(
    ...
    if experiment != "figure2":
        raise ValueError(f"runner only handles Figure 2, got {experiment!r}")
    config = load_config(experiment, phase, seeds)
    if namespace:
        config["artifact_namespace"] = namespace
...
def plot_experiment(
    experiment: str, phase: str = "full", namespace: str | None = None
) -> tuple[Path, Path]:
    if experiment != "figure2":
        raise ValueError(f"only Figure 2 supports standalone replotting, got {experiment!r}")
